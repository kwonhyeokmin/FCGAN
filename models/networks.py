...
from models.fcvgan import SFABlock, SFABlock2, SFABlock3
...
def get_scheduler(optimizer):
    ...
    if cfg.lr_policy == 'linear':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + 1 - cfg.n_epochs) / float(cfg.n_epochs_decay + 1)
            return lr_l
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = FourierComplexValueGenerator(input_nc, output_nc, ngf)
    else:
...
class FourierComplexValueGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        super(FourierComplexValueGenerator, self).__init__()
        ...
        # self.sfa1 = SFABlock(ngf=ngf)
        # self.sfa2 = SFABlock(ngf=ngf//2)
        # self.sfa1 = SFABlock2(ngf=ngf)
        # self.sfa2 = SFABlock2(ngf=ngf//2)
        # self.sfa1 = SFABlock3(in_channels=3, out_channels=ngf)
        # self.sfa2 = SFABlock3(in_channels=ngf, out_channels=ngf)
        # Attention GAN
    ...
    def forward(self, x):
        # Feature
        feature = F.pad(x, (3, 3, 3, 3), 'reflect')
        feature = F.relu(self.conv1_norm(self.conv1(feature)))
        feature = F.relu(self.conv2_norm(self.conv2(feature)))
        feature = F.relu(self.conv3_norm(self.conv3(feature)))
        ...
        output10 = x * attention10

        o = output1 + output2 + output3 + output4 + output5 + output6 + output7 + output8 + output9 + output10

        return o, output1, output2, output3, output4, output5, output6, output7, output8, output9, output10, \
            attention1, attention2, attention3, attention4, attention5, attention6, attention7, attention8, attention9, attention10, \
